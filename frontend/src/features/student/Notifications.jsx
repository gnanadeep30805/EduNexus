...
import { Card, Badge, Button, SkeletonCard, ErrorState, EmptyState } from '../../components/ui/index.jsx'
...
export default function Notifications() {
  ...
  const readAllMut = useMutation({
    ...
    onSuccess: () => { toast.success('All marked as read'); qc.invalidateQueries(['student', 'notifications']) },
    onError: err => toast.error(err.message),
  })
  const readOneMut = useMutation({
    mutationFn: id => api.patch(`/students/me/notifications/${id}/read`),
  ...
  return (
    <div>
      <div className="section-header">
        ...
        {unread > 0 && (
          <button className="btn btn-secondary btn-sm" onClick={() => readAllMut.mutate()} disabled={readAllMut.isPending}>
      ...
      {notifications.length === 0 ? (
        <Card className="card-padded">
          <EmptyState icon={Bell} title="No notifications" description="You're all caught up! Notifications about applications and matches will appear here." />
        </Card>
      ) : (
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: n.is_read ? 'var(--bg-surface)' : 'var(--accent-light)',
                  border: `1px solid ${n.is_read ? 'var(--border-base)' : 'hsl(221 83% 53% / 0.2)'}`,
                  cursor: n.is_read ? 'default' : 'pointer',
                }}
                onClick={() => !n.is_read && readOneMut.mutate(n.id)}
              >
                <div style={{
                  width: 34, height: 34, borderRadius: 'var(--radius-md)', flexShrink: 0,
                  background: n.is_read ? 'var(--bg-overlay)' : 'var(--accent-primary)',
                  color: n.is_read ? 'var(--text-tertiary)' : 'white',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: '0.8125rem', color: 'var(--text-secondary)', lineHeight: 1.5 }}>{n.body || n.message}</div>
